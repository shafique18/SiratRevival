...
export default function DesktopMenu({ menus, activeMegaMenu, setActiveMegaMenu }) {
  ...
  return (
    <ul className="hidden md:flex items-center gap-2 flex-grow justify-end" role="menubar">
      {menus.map((item) => (
        <li
          key={item.title}
          className="relative"
        ...
        >
          ...
          <MegaMenuPanel item={item} activeMegaMenu={activeMegaMenu} setActiveMegaMenu={setActiveMegaMenu} />
        </li>
      ))}
